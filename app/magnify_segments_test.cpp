...
//	const Vertex& v2 = triMesh.V.at(id2);
	...
	MeshFileWriter cubeWriter(cubeMesh, "cube1.vtk");
	cubeWriter.WriteMeshFile();
	///////////////////////////////////////////////
	// magnify cube *2
	...
	/////////////////////////////////////////
	std::ofstream opofs("opofs.xyz");
	std::ofstream ppofs("ppofs.xyz");
	std::ofstream mpofs("mpofs.xyz");
	std::ofstream cpofs("cpofs.xyz");
	std::ofstream cutofs("cut.xyz");

	for (size_t i = 0; i < point_segment0.size(); i++)
	{
		int vid = point_segment0.at(i);
		const Vertex& v = triMesh.V.at(vid);
		opofs << v.x << " " << v.y << " " << v.z << std::endl;
		const Point& vp = projectPoints.at(i);
		ppofs << vp.x() << " " << vp.y() << " " << vp.z() << std::endl;
	}

	for (size_t i = 0; i < 8; i++)
	{
		const glm::vec3& v = cube2[i];
		cpofs << v.x << " " << v.y << " " << v.z << std::endl;
	}

	for (size_t i = 0; i < points.size(); i++)
	{
		const Point& v = points[i];
		cutofs << v.x() << " " << v.y() << " " << v.z() << std::endl;
	}
	///////////////////////////////////////////////
	// parametrize vertex
	...
	MeshFileWriter meshWriter1(triMesh, "before.vtk");
	meshWriter1.WriteMeshFile();

	for (size_t i = 0; i < point_segment0.size(); i++)
	{
		int vid = point_segment0.at(i);
		const Vertex& v = triMesh.V.at(vid);
		mpofs << v.x << " " << v.y << " " << v.z << std::endl;
	}
