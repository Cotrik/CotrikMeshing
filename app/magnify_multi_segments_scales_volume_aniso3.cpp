...
void MapToCube(const Mesh& mesh, const std::vector<unsigned long>& pointIds, const glm::vec3 cube[8], const std::vector<int>& tetId, const std::vector<glm::vec3>& w,
        std::vector<glm::vec3>& magnified_point_segments_pos);
...
int main(int argc, char* argv[])
{
    if (argc != 6)
    {
        std::cout << "Usage: magnify_multi_segments_scales_volume_aniso2 input_tet_file input_tri_seg_file output_tet_file segmentid.txt scales.txt" << std::endl;
        return -1;
    }
    ...
    /////////////////////////////////////////
    std::vector<unsigned long> orig_point_segments_ids;
    std::vector<glm::vec3> magnified_point_segments_pos;
...
//	std::cout << "max_dis_id = " << max_dis_id << std::endl;
        ...
        ////////////////////////////////////////////////////////
//        std::vector<unsigned long> point_segment0_tet;
//        for (size_t i = 0; i < tetMesh.V.size(); i++)
//        {
//            const Vertex& v = tetMesh.V.at(i);
//            const double distance = a * v.x + b * v.y + c * v.z + d;
//            if (distance >= 0)
//                point_segment0_tet.push_back(i);
//        }
        std::vector<unsigned long> point_segment0_tet = point_segment0;
        ////////////////////////////////////////////////////////
...
//        cubeWriter.WriteFile();
        ...
        double scale1 = 3, scale2 = 1, scale3 = 2;
        if (seg_id == 1)
        {
            scale1 = 3, scale2 = 2, scale3 = 1;
        }
...
//		scale = 0.5;
        ...
        glm::vec3 cube2[8] =
        {
            glm::vec3(v1m.x + dir3.x * size3, v1m.y + dir3.y * size3, v1m.z + dir3.z * size3),
            glm::vec3(v2m.x + dir3.x * size3, v2m.y + dir3.y * size3, v2m.z + dir3.z * size3),
            glm::vec3(v2m.x - dir3.x * size3, v2m.y - dir3.y * size3, v2m.z - dir3.z * size3),
            glm::vec3(v1m.x - dir3.x * size3, v1m.y - dir3.y * size3, v1m.z - dir3.z * size3),
        };
        for (int i = 4; i < 8; i++)
        ...
        //set_handle_points(tetMesh, point_segment0_tet);
        orig_point_segments_ids.insert(orig_point_segments_ids.end(), point_segment0_tet.begin(), point_segment0_tet.end());
        Parametrize(tetMesh, point_segment0_tet, cube, tetIds, w);
        // MapToCube(tetMesh, point_segment0_tet, cube2, tetIds, w);
        std::vector<glm::vec3> newPos;
        MapToCube(tetMesh, point_segment0_tet, cube2, tetIds, w, newPos);
        magnified_point_segments_pos.insert(magnified_point_segments_pos.end(), newPos.begin(), newPos.end());
//        update_vertices(tetMesh);
    }

    set_handle_points(tetMesh, orig_point_segments_ids);
    for (int i = 0; i < orig_point_segments_ids.size(); i++)
    {
        unsigned long id = orig_point_segments_ids.at(i);
        Vertex& v = tetMesh.V.at(id);
        v.x = magnified_point_segments_pos.at(i).x;
        v.y = magnified_point_segments_pos.at(i).y;
        v.z = magnified_point_segments_pos.at(i).z;
    }
    update_vertices(tetMesh);
    ///////////////////////////////////////////////
    MeshFileWriter meshWriter(tetMesh, argv[3]);
...
{
    std::cout << "MapToCube" << std::endl;
    magnified_point_segments_pos.resize(pointIds.size());
    for (int i = 0; i < pointIds.size(); i++)
    {
        const Vertex& v = mesh.V.at(pointIds.at(i));
        int k = tetId.at(i);
...
//        v.y = w[i].x * p0.y + w[i].y * p1.y + w[i].z * p2.y + (1.0 - w[i].x - w[i].y - w[i].z) * p3.y;
//        v.z = w[i].x * p0.z + w[i].y * p1.z + w[i].z * p2.z + (1.0 - w[i].x - w[i].y - w[i].z) * p3.z;
        magnified_point_segments_pos[i].x = w[i].x * p0.x + w[i].y * p1.x + w[i].z * p2.x + (1.0 - w[i].x - w[i].y - w[i].z) * p3.x;
        magnified_point_segments_pos[i].y = w[i].x * p0.y + w[i].y * p1.y + w[i].z * p2.y + (1.0 - w[i].x - w[i].y - w[i].z) * p3.y;
        magnified_point_segments_pos[i].z = w[i].x * p0.z + w[i].y * p1.z + w[i].z * p2.z + (1.0 - w[i].x - w[i].y - w[i].z) * p3.z;
    }
}
