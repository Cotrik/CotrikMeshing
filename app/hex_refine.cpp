...
int main(int argc, char* argv[])
{
	...
	{
		std::cout << "Usage: hex_refine <input_hex_file> <output_hex_file> <iternum> " << std::endl;
		return -1;
	}
	...
	if (iternum == 1)
	{
		...
		const Mesh& hex_mesh = reader.GetMesh();

		Mesh new_mesh(hex_mesh);
		/*
		Mesh new_mesh1(hex_mesh);
		if (clockwise != 0)
		{
			std::cout << "clockwise oriented" << std::endl;
			for (int i = 0; i < new_mesh1.C.size(); i++)
			{
				Cell& c = new_mesh1.C.at(i);
				swap(c.at(1), c.at(3));
				swap(c.at(5), c.at(7));
			}
		}
		//Mesh new_mesh(new_mesh1);
		Mesh new_mesh(new_mesh1.V, new_mesh1.C, MESH_TYPE_HEXAHEDRON_VTK);
		*/

/*		std::vector<unsigned long> new_edge_vertex_index(new_mesh.E.size());
		std::vector<unsigned long> new_face_vertex_index(new_mesh.F.size());
		std::vector<unsigned long> new_hex_vertex_index(new_mesh.C.size());
		for (size_t i = 0; i < new_edge_vertex_index.size(); i++)new_mesh.Cnew_mesh.Cnew_mesh.C
		{
			new_edge_vertex_index.at(i) == new_mesh.V.size() + i;
		}
		for (size_t i = 0; i < new_face_vertex_index.size(); i++)
		{
			new_face_vertex_index.at(i) == new_mesh.V.size() + new_edge_vertex_index.size() + i;
		}
		for (size_t i = 0; i < new_hex_vertex_index.size(); i++)new_mesh.Cnew_mesh.Cnew_menew_mesh.Cnew_mesh.Cnew_mesh.Cnew_mesh.Cnew_mesh.Cnew_mesh.Csh.C
		{
			new_hex_vertex_index.at(i) == new_mesh.V.size() + new_edge_vertex_index.size() + new_face_vertex_index.size() + i;
		}
		*/
		////////////////////////////////////////////////////////////////////////////
		if (!new_mesh.C.empty())
