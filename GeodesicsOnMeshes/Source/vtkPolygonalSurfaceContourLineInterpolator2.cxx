...
//----------------------------------------------------------------------
int vtkPolygonalSurfaceContourLineInterpolator2::InterpolateLine(vtkPolyData *polyData,	vtkIdType beginVertId, vtkIdType endVertId, std::vector<vtkIdType>& ids)
{
	double p[3];

	vtkSmartPointer<vtkIdList> vertexIds = NULL;

	vtkDijkstraGraphGeodesicPath *dggp = vtkDijkstraGraphGeodesicPath::SafeDownCast(this->GeodesicPath);
	vtkFastMarchingGeodesicPath *fmgp = vtkFastMarchingGeodesicPath::SafeDownCast(this->GeodesicPath);

	if (this->GeodesicMethod == vtkPolygonalSurfaceContourLineInterpolator2::DijkstraMethod)
	{
		// Compute the shortest path through the surface mesh along its edges
		// using Dijkstra.
	...
	if (this->DistanceOffset != 0.0)
	{
		vertexNormals = polyData->GetPointData()->GetNormals();
	}
