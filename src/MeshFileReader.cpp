...
void MeshFileReader::ReadMeshFile()
{
    ...
    if (str == "Hexahedra") {C.resize(cnum, Cell(8)); m_mesh.m_cellType = HEXAHEDRA; csize = 8;}
    else if (str == "Tetrahedra") {C.resize(cnum, Cell(4)); m_mesh.m_cellType = TETRAHEDRA; csize = 4;}
    else if (str == "Quadrilaterals") {C.resize(cnum, Cell(4)); m_mesh.m_cellType = QUAD; csize = 4;}
    else if (str == "Triangles") {C.resize(cnum, Cell(3)); m_mesh.m_cellType = TRIANGLE; csize = 3;}
    else if (str == "Edges") {
        while (ifs >> str)
            if (str == "Hexahedra")
                break;
...
const MESH_TYPE& MeshFileReader::GetMeshType() const
{
	return m_meshType;
}
...
void MeshFileReader::GetScalarFields()
{
	...
	if (scalarDataInt)
	{
		...
		m_mesh.scalarFields.push_back(scalarField);
	}
